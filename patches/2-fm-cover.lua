...
local function isTitleVisible()
    return getBool("vc_show_tixintle", true) or getBool("vc_show_author", true)
end
...
function MosaicMenuItem:update(...)
    ...
    if not (self.entry.is_file or self.entry.file) and self.mandatory then
        ...
        folder_name = folder_name:gsub("^=", "")   -- 去掉分组/排序用的前导 "=" 标记
...
userpatch.registerPatchPluginFunc("coverbrowser", function(plugin)
    ...
    local ok_order, order = pcall(require, "ui/elements/filemanager_menu_order")
    if ok_order and order.filemanager_settings then
        local exists = false
        for _, k in ipairs(order.filemanager_settings) do
            if k == "vc_cover_settings" then exists = true break end
        end
        if not exists then
            table.insert(order.filemanager_settings, "vc_cover_settings")
        end
    end
